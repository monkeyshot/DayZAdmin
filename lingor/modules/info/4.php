...
while ($row=mysql_fetch_array($res)) {
	...
	$owneruid = "";
	if ($row['oid'] != "0"){
		$query = "SELECT * FROM (select profile.name, survivor.* from profile, survivor as survivor where profile.unique_id = survivor.unique_id) as T where id = ".$row['oid']." LIMIT 1"; 
